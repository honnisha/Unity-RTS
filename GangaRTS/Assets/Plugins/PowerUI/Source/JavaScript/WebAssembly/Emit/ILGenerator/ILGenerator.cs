...
namespace WebAssembly
{
	...
	public abstract class ILGenerator
	{
		...
		/// <summary>True if this generator is in 64 bit mode (WebAssembly MVP is 32 only).</summary>
		public bool X64;
		
		/// <summary>
		/// The runtime the method is for. Either a ScriptEngine or a WebAssembly.Module.
		/// </summary>
		public Runtime Runtime;

		/// <summary>
		/// If we're generating an anonymous function then we have a special 'this' field
		/// in the object being passed to it. Anonymous functions are actually members of
		/// special classes (same as C#) so they can handle scoping.
		/// </summary>
		public System.Reflection.FieldInfo InstanceThis;
		
		/// <summary>The current WebAssembly instruction.</summary>
		public WebAssembly.Instruction Instruction;
		
		/// <summary>The current WebAssembly reader.</summary>
		public WebAssembly.Reader Reader{
			get{
				return (Runtime as WebAssembly.Module).Reader;
			}
		}
		...
		/// <summary>The current WebAssembly function body, if there is one.</summary>
		public WebAssembly.FunctionBody FunctionBody;
		...
		/// </summary>
		public abstract void Complete(string name, Type[] arguments, Type returnType, bool emitReturn);
		...
		public abstract ILLocalVariable DeclareVariable(Type type, string name);
		
		public ILLocalVariable DeclareVariable(Type type)
		{
		...
		/// <summary>
		/// Pops two values from the stack, divides the first by the second (unsigned), then pushes the
		/// result to the stack.
		/// </summary>
		...
		/// </summary>
		public abstract void CallIndirect(Type returnType,Type[] parameterTypes);
