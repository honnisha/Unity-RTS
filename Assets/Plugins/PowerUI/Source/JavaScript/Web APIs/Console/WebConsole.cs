...
namespace JavaScript
{
	...
	public partial class console
	{
		...
		/// </summary>
		internal IconsoleOutput Output
		{
		...
		{
			log(consoleMessageStyle.Regular, items);
		}
		...
		{
			log(consoleMessageStyle.Information, items);
		}
		...
		{
			log(consoleMessageStyle.Warning, items);
		}
		...
		{
			log(consoleMessageStyle.Error, items);
		}
		...
		public void assert(bool expression, params object[] items)
		{
			if (expression == false)
			{
				if (items.Length > 0)
				{
					...
					System.Array.Copy(formattedItems, 0, formattedItems2, 1, formattedItems.Length);
		...
		public void timeEnd(string name)
		{
			...
			if (string.IsNullOrEmpty(name))
				log(consoleMessageStyle.Regular, string.Format("{0}ms", stopwatch.ElapsedMilliseconds));
			else
				log(consoleMessageStyle.Regular, string.Format("{0}: {1}ms", name, stopwatch.ElapsedMilliseconds));
			...
			if (items[0] is string)
			{
				...
				while (items.Length > itemsConsumed)
				{
					...
					switch (patternType)
					{
						...
						case 'i':
							double number;
							double.TryParse(items[itemsConsumed++].ToString(), out number);
