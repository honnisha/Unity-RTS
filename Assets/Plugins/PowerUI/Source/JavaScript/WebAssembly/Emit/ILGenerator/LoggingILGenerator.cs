...
namespace WebAssembly
{
	...
	public class LoggingILGenerator : ILGenerator
	{
		...
		public override void Complete(string name, Type[] arguments, Type returnType, bool emitReturn)
		{
			...
			this.generator.Complete(name,arguments,returnType,emitReturn);
			...
			if(arguments!=null){
				for(int i=0;i<arguments.Length;i++){
					...
					args+=arguments[i]+" arg"+(i+1);
				}
			}
			
			WebAssembly.Module.Log("-IL "+name+"("+args+") : "+returnType+"-");
			WebAssembly.Module.Log(il);
		...
		{
			Log("ldc.r4", value);
			this.generator.LoadSingle(value);
		}
		...
		{
			Log("rem.un");
			this.generator.RemainderUnsigned();
		}
		...
		{
			Log("conv.r4");
			this.generator.ConvertToSingle();
		}
		...
		public override void Unaligned(byte align){
			Log("unaligned\t"+align);
			this.generator.Unaligned(align);
		}
		...
		public override void AddressUnsignedInt8(){
			Log("ldind.i1.un");
			this.generator.AddressUnsignedInt8();
		}
		...
		public override void AddressUnsignedInt16(){
			Log("ldind.i2.un");
			this.generator.AddressUnsignedInt16();
		}
		...
		public override void AddressUnsignedInt32(){
			Log("ldind.i4.un");
			this.generator.AddressUnsignedInt32();
		}
		...
		public override void AddressUnsignedInt64(){
			Log("ldind.i8.un");
			this.generator.AddressUnsignedInt64();
		}
		...
		public override void CallIndirect(Type returnType,Type[] parameterTypes){
			Log("calli(): "+returnType.ToString());
			this.generator.CallIndirect(returnType,parameterTypes);
		}
		...
		{
			LogInstruction(instruction, "\""+stringConstant+"\"");
		}
		...
		{
			LogInstruction(instruction, field.FieldType+" "+field.Name+"/"+field.DeclaringType);
		}
